...
function MoveSample_gcs30
    ...
    try
        ...
        closed_loop = 2;
        open_loop = 1;        
        SetModeOfOperation (PIdevice,axis, closed_loop)
        %% Get min/max position limits 
        % determine the allowed travel range of the stage
        minimumPosition = GetMinPositionLimit(PIdevice,axis);
        maximumPosition = GetMaxPositionLimit(PIdevice,axis);
        travelRange = ( maximumPosition - minimumPosition );

        %% Do absolute motions
        targetPosition = minimumPosition + (travelRange * 0.1);
        MoveToAbsoluteTarget(PIdevice,axis,targetPosition)
        ...
        RelativeMove(PIdevice,axis,offset)   
        WaitForMotionDone(PIdevice)
        fprintf('Successfully executed relative move backward by %f units\n', offset);
        
        offset = offset * -1;
        RelativeMove(PIdevice,axis,offset) 
        WaitForMotionDone(PIdevice)
        fprintf('Successfully executed relative move forward by %f units\n', offset);
    
        SetModeOfOperation (PIdevice,axis,open_loop)
...
function  MoveToAbsoluteTarget(PIdevice,axis,targetPosition)
...
function RelativeMove(PIdevice,axis,offset)
    currentTarget = PIdevice.qMOV(axis);
