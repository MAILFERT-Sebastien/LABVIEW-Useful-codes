...
def save_binary(
    ...
    """
    Sauvegarde les tableaux en binaire pur (float64 / uint8, row-major C)
    et ecrit sequence_info.json avec toutes les metadonnees.

    Lecture en LabVIEW :
      Read Binary File.vi
        -> type : DBL (ao_master/ao_slave) ou U8 (do_master)
        -> count : N_samples x N_cols
      Reshape 1D -> 2D avec Reshape Array [N_samples, N_cols]
      Chaque colonne = une voie DAQ
    """
    out_dir.mkdir(parents=True, exist_ok=True)

    # Ecriture row-major (C order) pour correspondre a LabVIEW row-major
    ao_master.astype(np.float64).tofile(out_dir / "ao_master.bin")
    ao_slave.astype(np.float64).tofile(out_dir  / "ao_slave.bin")
    do_master.astype(np.uint8).tofile(out_dir   / "do_master.bin")
    ...
    info = {
        ...
        "files": {
            "ao_master": {
                "filename":  "ao_master.bin",
                "dtype":     "float64",
                "shape":     list(ao_master.shape),
                "columns":   [
                    "col0: AO0#1 laser_405nm (V)",
                    "col1: AO1#1 XY_X (reserve 0V)",
                    "col2: AO2#1 Pockels (V)",
                    "col3: AO3#1 Z_piezo (V)",
                ],
            },
            "ao_slave": {
                "filename":  "ao_slave.bin",
                "dtype":     "float64",
                "shape":     list(ao_slave.shape),
                "columns":   [
                    "col0: AO0#2 laser_488nm (V)",
                    "col1: AO1#2 laser_561nm (V)",
                    "col2: AO2#2 laser_640nm (V)",
                ],
            },
            "do_master": {
                "filename":  "do_master.bin",
                "dtype":     "uint8",
                "shape":     list(do_master.shape),
                "columns":   [
                    "col0: P0.0 pulse_filtre",
                    "col1: P0.1 laser_405_DO (reserve 0)",
                    "col2: P0.2 laser_455_DO (reserve 0)",
                    "col3: P0.3 blanking (1=lasers OFF)",
                    "col4: P0.4 trigger_camera",
                    "col5: P0.5 laser_488_DO (reserve 0)",
                    "col6: P0.6 laser_561_DO",
                    "col7: P0.7 laser_640_DO (reserve 0)",
                ],
            },
        },
        "labview_read": {
            "ao_master": f"Read Binary File DBL, count={ao_master.shape[0]*ao_master.shape[1]}, then Reshape Array [{ao_master.shape[0]}, {ao_master.shape[1]}]",
            "ao_slave":  f"Read Binary File DBL, count={ao_slave.shape[0]*ao_slave.shape[1]}, then Reshape Array [{ao_slave.shape[0]}, {ao_slave.shape[1]}]",
            "do_master": f"Read Binary File U8, count={do_master.shape[0]*do_master.shape[1]}, then Reshape Array [{do_master.shape[0]}, {do_master.shape[1]}]",
        },
        "acquisition": {
            "NPOS":              p["NPOS"],
            "NZ":                p["NZ"],
    ...
    print(f"[OK] ao_master.bin   shape={ao_master.shape}  dtype=float64"
          f"  ({ao_master.nbytes/1e6:.1f} MB)")
    print(f"[OK] ao_slave.bin    shape={ao_slave.shape}  dtype=float64"
          f"  ({ao_slave.nbytes/1e6:.1f} MB)")
    print(f"[OK] do_master.bin   shape={do_master.shape}  dtype=uint8"
          f"  ({do_master.nbytes/1e6:.1f} MB)")
